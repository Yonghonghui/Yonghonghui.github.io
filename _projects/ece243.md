...
- **Architectural Optimization (GRU $\to$ LSTM):**
  ...
  - Integrated a **"Linderman" Post-Stack** (`Linear` $\to$ `LayerNorm` $\to$ `Dropout` $\to$ `GELU`) to increase representational capacity and stabilize gradient flow.

- **Loss Function Engineering:**
  - Implemented **Focal CTC Loss** ($L_{focal} = (1 - p_t)^\gamma \cdot L_{CTC}$) to address class imbalance.
